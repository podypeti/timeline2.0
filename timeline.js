...
// --- Safe rounded-rectangle helper (works with or without native ctx.roundRect) ---
function roundRect(ctx, x, y, w, h, r, fill = true, stroke = false) {
  let radius = typeof r === 'number' ? { tl: r, tr: r, br: r, bl: r } : (r || { tl: 0, tr: 0, br: 0, bl: 0 });
...
fetch('timeline-data.csv')
  ...
  .catch(err => {
    ...
    message('Failed to initialize the timeline. See console for details.');
  });
...
function parseCSV(text){ if(!text) return []; const raw=text.split(/\r?\n/); const lines=raw.map(l=>l.trim()).filter((l,i)=> (l.length>0)); if(lines.length===0) return []; const head=csvSplit(lines[0]).map(h=>h.trim()); const map={}; head.forEach((h,i)=>{ map[h.toLowerCase()]=i; }); const out=[]; for(let i=1;i<lines.length;i++){ const parts=csvSplit(lines[i]); if(!parts || parts.length===0) continue; const norm={}; Object.keys(map).forEach(k=>{ const idx=map[k]; const rawVal=(parts[idx]!==undefined?parts[idx]:''); const v=(''+rawVal).trim().replace(/^"(.*)"$/, '$1'); norm[k]=v; }); const title=norm.headline || norm.title || norm.name || '';
  const type=norm.type || '';
  const group=norm.group || '';
  const media=norm.media || '';
  const textField=norm.text || norm['display date'] || '';
  const y=parseInt(norm.year,10); const mo=norm.month; const d=norm.day; const timeStr=norm.time || norm['time'] || '';
  const hasMonth=!isNaN(parseInt(mo,10)); const hasDay=!isNaN(parseInt(d,10));
  let start=toTs(y,mo,d,timeStr); let startPrec=hasDay?'day':(hasMonth?'month':'year');
  const endYear=parseInt(norm['end year'],10); let end=NaN, endPrec=startPrec;
  if(!isNaN(endYear)){
    const emo=norm['end month']; const ed=norm['end day']; const et=norm['end time'] || '';
    const eHasMonth=!isNaN(parseInt(emo,10)); const eHasDay=!isNaN(parseInt(ed,10));
    end=toTs(endYear,emo,ed,et); endPrec=eHasDay?'day':(eHasMonth?'month':'year');
  }
  if(isNaN(start)){
...
function buildLegend(){ const host=document.getElementById('legend'); if(!host) return; host.innerHTML=''; chipIndex.clear(); availableGroups.forEach(({label,keyLower})=>{ const color=COLOR_MAP[keyLower] || COLOR_MAP['']; const chip=document.createElement('div'); chip.className='chip'+(activeGroups.has(keyLower)?'':' inactive'); chip.setAttribute('data-key', keyLower); chip.innerHTML='<span class="swatch" style="background:'+color+'"></span><span>'+label+'</span>';
  chip.addEventListener('click',()=>{ if(activeGroups.has(keyLower)) activeGroups.delete(keyLower); else activeGroups.add(keyLower); chip.classList.toggle('inactive'); applyFiltersAndPack(); draw(); });
  host.appendChild(chip); chipIndex.set(keyLower, chip);
}); }
function attachLegendToolbar(){ const btnAll=document.getElementById('btnAll'); const btnNone=document.getElementById('btnNone'); if(btnAll){ btnAll.onclick=()=>{ activeGroups.clear(); availableGroups.forEach(g=>activeGroups.add(g.keyLower)); chipIndex.forEach((el)=> el.classList.remove('inactive')); applyFiltersAndPack(); draw(); }; }
...
function linkifyScripture(text){ const tokens=String(text||'').split(/(\s+)/); let out=''; let lastBook=null; for(let i=0;i<tokens.length;){ const tk=tokens[i]; if(/\s+/.test(tk)){ out+=tk; i++; continue; } const next=tokens[i+1]||''; const next2=tokens[i+2]||''; if(BOOK_TOKEN_RE.test(tk) && /\s+/.test(next) && REF_RE.test(next2)){ lastBook=tk; const m=REF_RE.exec(next2); const code8=jwCodeFor(tk,m[1],m[2]); const label=tk+next+next2; out += code8 ? ( '<a target="_blank" rel="noopener" href="'+jwFinderUrl(code8)+'">'+escapeHtml(label)+'</a>' ) : escapeHtml(label); i+=3; continue; }
  ...
  if(BOOK_TOKEN_RE.test(tk)) lastBook=tk; out+=escapeHtml(tk); i++; }
...
function draw(){
  ...
  // Year ticks with dynamic step
  const msPerYear=365.2425*24*3600*1000; const approx=scale*msPerYear; let step; // in years
  ...
  rows.forEach((row,i)=>{
    ...
    row.forEach(ev=>{
      ...
      const pillW=Math.max(120, textW+padX*2); const pillH=padY*2 + heights.reduce((a,b)=>a+b,0) + (lines.length-1)*lineGap; let pillX=stickX+10; if(pillX+pillW>W-8) pillX=stickX-10-pillW; if(pillX<4) pillX=4; const pillY=yTop;
...
function packRows(){
  rows = [];
  const evs = [...visibleEvents].sort((a,b)=> a.start===b.start ? a.end-b.end : a.start-b.start);
  const MIN_GAP_MS = 0; // allow touching
  for(const ev of evs){
    let placed = false;
    for(const row of rows){
      const last = row[row.length-1];
      if((last?.end ?? -Infinity) <= (ev.start - MIN_GAP_MS)) { row.push(ev); placed = True; break; }
    }

// === Timeline with BCE/CE, filters, details, WOL links ===
...
function failMsg(msg){ canvas.width=canvas.clientWidth; canvas.height=canvas.clientHeight; ctx.clearRect(0,0,canvas.width,canvas.height); ctx.fillStyle='#000'; ctx.font='14px Arial'; ctx.fillText(msg, 10, 30); }
...
function linkifyScripture(text){ const src=escapeHtml(text); let out='', lastIdx=0, lastBook=null; REG_COMBINED.lastIndex=0; let m; while((m=REG_COMBINED.exec(src))!==null){ const idx=m.index; out+=src.slice(lastIdx, idx); if(m[1]&&m[2]){ lastBook=m[1].replace(/\s+/g,' ').trim(); const ref=m[2]; const q=encodeURIComponent(`${lastBook} ${ref}`); out+=`${WOL_SEARCH_BASE}${q}${escapeHtml(lastBook)} ${escapeHtml(ref)}</a>`; } else if(m[3]){ const ref=m[3]; if(lastBook){ const q=encodeURIComponent(`${lastBook} ${ref}`); out+=`${WOL_SEARCH_BASE}${q}${escapeHtml(ref)}</a>`; } else { out+=escapeHtml(ref); } } lastIdx=REG_COMBINED.lastIndex; }
  out+=src.slice(lastIdx); return out; }
function isImageUrl(url){ return /\.(png|jpe?g|gif|webp|avif|svg)(\?|#|$)/i.test(url); }

function draw(){ canvas.width=canvas.clientWidth; canvas.height=canvas.clientHeight; hitRegions=[]; const W=canvas.width, H=canvas.height; ctx.clearRect(0,0,W,H);
...
function wrapText(ctx,text,maxWidth,font='12px Arial'){ const words=String(text||'').split(/\s+/); const lines=[]; let line=''; ctx.font=font; words.forEach(w=>{ const test=line? (line+' '+w):w; if(ctx.measureText(test).width>maxWidth && line){ lines.push({text:line,font}); line=w; } else { line=test; } }); if(line) lines.push({text:line,font}); return lines; }
...
function showDetails(ev){ if(!detailsPanel||!detailsContent) return; const title=ev.title||'(untitled)'; const when=displayWhen(ev); const group=groupKeyFor(ev); const text=ev.text||''; const media=(ev.media||'').trim(); const mediaHtml = media ? (isImageUrl(media)? f`<div class="media">`+`</div>` : f`<div class="media">`+`</div>`) : '';
...
function f(strings,...vals){ // small helper for safer string concat

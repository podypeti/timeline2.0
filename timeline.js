// === Robust timeline.js (regex-safe) ===
// Config: WOL language segment (change to 'en/lp-e' for English)
...
function parseCSV(text){ if(!text) return []; const raw=text.split(/\r?\n/); const lines=raw.map(l=>l.trim()).filter((l,i)=>(l.length>0)||i===0); if(lines.length===0) return []; const head=csvSplit(lines[0]).map(h=>h.trim()); const map={}; head.forEach((h,i)=>{ map[h.toLowerCase()]=i; }); const out=[]; for(let i=1;i<lines.length;i++){ const parts=csvSplit(lines[i]); if(!parts||parts.length===0) continue; const norm={}; Object.keys(map).forEach(k=>{ const idx=map[k]; const rawVal=(parts[idx]!==undefined?parts[idx]:''); const v=(''+rawVal).trim().replace(/^\"(.*)\"$/, '$1'); norm[k]=v; }); const title=norm.headline||norm.title||norm.name||''; const type=norm.type||''; const group=norm.group||''; const media=norm.media||''; const textField=norm.text||norm['display date']||''; const year=parseInt(norm.year,10); const month=norm.month; const day=norm.day; const timeStr=norm.time||norm['time']||''; let start=toTs(year,month,day,timeStr); const endYear=parseInt(norm['end year'],10); let end=NaN; if(!isNaN(endYear)){ const endMonth=norm['end month']; const endDay=norm['end day']; const endTime=norm['end time']||''; end=toTs(endYear,endMonth,endDay,endTime); } if(isNaN(start)){ const single=norm.start||norm['start date']||norm['display date']; if(single){ const d=new Date(single); if(!isNaN(d)) start=d.getTime(); } } if(isNaN(start)) continue; if(isNaN(end)) end=start; if(end<start){ const tmp=start; start=end; end=tmp; } out.push({title, start, end, type, group, media, text:textField, raw:norm}); } return out; }
...
function formatDateHuman(ts){ const d=new Date(ts); const y=d.getUTCFullYear(); const m=d.getUTCMonth()+1; const day=d.getUTCDate(); const yTxt=(y<0)?String(Math.abs(y))+'\u202fBCE':String(y); return yTxt + ((m&&day)? ('-'+String(m).padStart(2,'0')+'-'+String(day).padStart(2,'0')) : ''); }
function displayWhen(ev){ if(ev.raw&&ev.raw['display date']) return ev.raw['display date']; const a=formatDateHuman(ev.start); const b=formatDateHuman(ev.end); return (a===b)?a:(a+' — '+b); }
...
// ---- SCRIPTURE LINKING (ultra-safe; no complex regex) ----
// Very tolerant matcher: Book abbreviations (with optional dot) then space then ch:vs
// If you prefer to disable linking entirely for now, set ENABLE_LINKIFY=false.
const ENABLE_LINKIFY = true;
const BOOK_WORDS = ['Gen','Ex','Lev','Num','Deut','Josh','Jg','Judg','Ruth','1 Sam','2 Sam','1 Ki','2 Ki','1 Ch','2 Ch','Chron','Ezra','Neh','Esth','Job','Ps','Prov','Eccl','Song','Isa','Jer','Lam','Ezek','Dan','Hos','Joel','Amos','Obad','Jonah','Mic','Nah','Hab','Zeph','Hag','Zech','Mal','Matt','Mt','Mark','Mrk','Luke','Lu','John','Jn','Acts','Rom','Ro','1 Cor','2 Cor','Gal','Eph','Phil','Col','1 Thess','2 Thess','1 Tim','2 Tim','Titus','Philem','Heb','Jas','James','1 Pet','2 Pet','1 John','2 John','3 John','Jude','Rev'];
const BOOK_RE = /^(Gen|Ex|Lev|Num|Deut|Josh|Jg|Judg|Ruth|1\s?Sam|2\s?Sam|1\s?Ki|2\s?Ki|1\s?Ch|2\s?Ch|Chron|Ezra|Neh|Esth?|Job|Ps|Prov|Eccl|Song|Isa|Jer|Lam|Ezek|Dan|Hos|Joel|Amos|Obad|Jonah?|Mic|Nah|Hab|Zeph|Hag|Zech|Mal|Matt|Mt|Mark|Mrk|Luke|Lu|John|Jn|Acts|Rom|Ro|1\s?Cor|2\s?Cor|Gal|Eph|Phil|Col|1\s?Thess|2\s?Thess|1\s?Tim|2\s?Tim|Titus|Philem|Heb|Jas|James|1\s?Pet|2\s?Pet|1\s?John|2\s?John|3\s?John|Jude|Rev)\.?$/i;
const REF_RE = /^\d+:\d+([\-–]\d+)?$/;
function escapeHtml(s){ return String(s||'').replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }
function linkifyScripture(text){ if(!ENABLE_LINKIFY) return escapeHtml(text); const tokens = String(text||'').split(/(\s+)/); let out=''; let lastBook=null; for(let i=0;i<tokens.length;i++){ const tk=tokens[i]; if(/\s+/.test(tk)){ out+=tk; continue; } const safe=escapeHtml(tk); if(BOOK_RE.test(tk)){ lastBook=tk.replace(/\s+/g,' ').replace(/\.$/,''); out+=safe; continue; } if(REF_RE.test(tk) && lastBook){ const q=encodeURIComponent(lastBook+' '+tk); out+= '<a target="_blank" rel="noopener" href="'+WOL_SEARCH_BASE+q+'">'+safe+'</a>'; continue; } out+=safe; } return out; }
